...
class FirebaseService {
  ...
  /// Maps the signed-in Supabase user into local [AppUser], preserving profile if same account.
  Future<void> ensureUserFromSupabaseSession(User supabaseUser) async {
    final meta = supabaseUser.userMetadata;
    final name = (meta?['full_name'] ?? meta?['name'] ?? 'User').toString();
    final email = supabaseUser.email ?? '';
    final uid = supabaseUser.id;

    final existing = getCurrentUser();
    if (existing != null && existing.uid == uid) {
      await updateProfile(existing.copyWith(email: email, name: name));
      return;
    }
    ...
    if (_db.getActivities().isEmpty) {
      await _db.addActivity(ActivityLog(
        id: '1',
        steps: 4280,
        calories: 1840,
        distance: 3.2,
    ...
    if (_db.getHealthMetrics().isEmpty) {
      await _db.addHealthMetric(HealthMetric(
        id: '1',
        heartRate: 72,
        spO2: 98,
        bodyTempC: 36.6,
  ...
  Future<void> logOut() async {
    // Session is cleared in AuthService via Supabase; local profile may remain for offline UX.
  }
